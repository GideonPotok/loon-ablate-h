"""
Ablation I — Linear Potential Shaping (v2 server, 3600 episodes):

Question this answers:
  "Does replacing the exponential shaping potential (tau=2R) with a linear
   potential (D_max=10R) give the agent a constant-gradient recovery signal
   when it drifts far outside the 50 km station-keeping radius?"

Problem with Ablations D–H:
  The exponential shaping potential Φ(s) = β·exp(-d/tau) with tau=2R=100 km
  becomes effectively flat beyond ~5R (250 km). The agent receives no useful
  gradient signal for long-range recovery. The binary per-step reward used in
  D–H is even sparser for distant states.

Options considered:
  1. Widen exponential tau (e.g. 5R→10R): ~3× better than v1 at 300 km, but
     still decays and loses precision near the boundary.
  2. Linear potential, D_max=10R (chosen): Φ(s) = β·max(0, 1 - d/D_max).
     Gives constant per-km gradient identical at 50 km and 400 km. Ng-invariant
     — optimal policy unchanged. No downsides.
  3. Piecewise linear reward (not potential): constant gradient but biases
     toward shorter episodes if k too large.

Change vs Ablation H:
  - Reward: v2 server, binary per-step + linear potential shaping
    (shaping_linear=True, shaping_D_max=500_000 m = 10R)
  - State: 20-dim (same as H — use_expanded_state=False)
  - Architecture: [128, 64] (same)
  - Curriculum: 3600 eps/worker (same)

Usage:
    python ablate_i_train.py
"""
...
LOG_PATH       = Path('/tmp/train_ablate_i.log')
...
def main():
    ...
    tee('═' * 78)
    tee('ABLATION I: linear potential shaping — v2 server, 20-dim state, 3600 eps')
    tee('═' * 78)
    ...
    tee(f'Change vs H: reward = binary per-step + linear potential (was exponential tau=2R)')
    tee('─' * 78)
