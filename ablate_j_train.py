"""
Ablation J — Recovery Spawn Training (v2 server, 3600 episodes):

Question this answers:
  "Does explicitly training the agent from far-away initial states (150–500 km)
   give it a recovery policy that prevents the escape-and-never-return failure
   observed in Ablation I?"

Problem with Ablation I:
  The agent stations-keeps well for ~20h then escapes to 700–950 km and never
  returns. Root cause: a data coverage gap — 50% of training episodes end at
  ≤12h, so the agent never trains on recovery from distant states. The 20h
  escape coincides with the IGW phase shift (8h period) making the previously-
  good altitude stop working; with no recovery experience, the agent thrashes.

Change vs Ablation I:
  - Recovery spawning: 30% of 24h+ training episodes spawn the balloon
    150–500 km from station instead of the default 30 km.
  - Everything else identical: v2 server, binary reward, linear potential
    shaping (D_max=10R=500 km), 20-dim state, [128,64] arch, 3600 eps/worker.

Why 24h+ only:
  Short episodes (≤12h) don't have enough time for meaningful recovery from
  150+ km — applying spawning there would only add noise. 24h+ episodes give
  the agent 24–72h to practice finding the return altitude.

Why 150–500 km:
  150 km = 3R: meaningfully outside, recoverable in 24h. 500 km = D_max = 10R:
  the shaping horizon, hardest case. Uniform sampling covers the full range so
  the agent builds a recovery policy across all distances, not just worst-case.

Usage:
    python ablate_j_train.py
"""
...
# ── Recovery spawn parameters ─────────────────────────────────────────────────

RECOVERY_SPAWN_PROB    = 0.30      # 30% of eligible episodes use far spawn
RECOVERY_SPAWN_MIN_KM  = 150.0    # minimum spawn distance (3R)
RECOVERY_SPAWN_MAX_KM  = 500.0    # maximum spawn distance (10R = D_max)
RECOVERY_SPAWN_MIN_DUR = 3600 * 24  # only episodes ≥ 24h are eligible

BASE_CONFIG = QRConfig(
    state_dim         = 20,            # v1-compatible 20-dim state
    hidden_sizes      = [128, 64],
    action_count      = 17,
    ...
    device            = 'cpu',
    # v2 server flags (handled via _env_flags(), not QRConfig)
...
LOG_PATH       = Path('/tmp/train_ablate_j.log')
...
def _eval_multi_preset(agent: QRAgent, ep: int, seed: int,
    ...
    for pi, preset in enumerate(PRESETS):
        ...
        for r in range(n_runs):
            ...
            state = env.reset()   # eval always spawns at default 30 km
            done = False
            twr50 = 0.0
            while not done:
...
def worker_fn(worker_id: int, result_queue: mp.Queue, max_episodes: int = 0):
    ...
    for ep in range(n_eps):
        ...
        # Recovery spawn: 30% of 24h+ episodes start the balloon 150–500 km out.
        spawn_km = None
...
def main():
    ...
    tee('═' * 78)
    tee('ABLATION J: recovery spawn training — v2 server, 20-dim state, 3600 eps')
    tee('═' * 78)
    ...
    tee(f'Shaping:     linear  β=0.5  γ=0.97  D_max=500 km (10R)')
    ...
    tee(f'Change vs I: recovery spawn training (all other settings identical)')
    tee('─' * 78)
