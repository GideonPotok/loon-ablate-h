...
- [Architecture: Gas-Balloon (Helium/Sand) Env Variant](architecture/gassand-env.md) — the zero-pressure gas-balloon model (drop sand → rise, vent helium → sink; both finite and irreversible; release amount → drift speed), its two-regime lift physics, the 11-action release ladder and 21-dim state, the multi-seed transfer probe, and the flag-gated resource-aware reward.
