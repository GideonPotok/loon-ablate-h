"""
Ablation Q — Option-Critic with GRU, per-step training cadence (v2, 3600 eps):

Question this answers:
  "Was M's failure the per-episode training starvation, not the option-critic
   architecture? M trained once per seq_train (16) environment steps *after*
   each episode ended — ~32x fewer gradient steps than L's 2-per-env-step
   cadence on the same data. O bundled a revert of M's lr/batch_size with new
   clipping and regressed below M; P's gradient-norm probe (948 steps, median
   0.74, max 1.26, 0% > 5.0) ruled clipping in/out as a factor entirely.
   Nobody has yet run M's architecture with L's optimization recipe.

   Q is that experiment: M's exact architecture (4 options, GRU-64,
   burn-in 16 + train 16 sequence windows) trained inside the env-step loop
   like L, with L's lr (1e-4) and L's target-update period (15 episodes).
   If the cadence theory is right, Q should close most of the gap to L
   (46.9%) with far less seed variance than M (stdev 9.9pp); if Q still
   scores like M (24.7%) despite 8x the gradient steps and ~4x L's data
   throughput, that is real evidence against option-critic on this problem."

Changes vs Ablation M/P (all optimization recipe, zero architecture):
  - Training cadence: per env step (every TRAIN_EVERY_STEPS steps, sampling
    the sequence buffer), not per episode. Episodes are still pushed whole.
  - learning_rate: 3e-4 -> 1e-4  (L's value; M's 3e-4 compensated starvation)
  - target_update_freq: 30 -> 15 episodes  (L's value)
  - grad_clip_norm: None  (L has no clipping; P's probe showed it is inert)
  - batch_size stays 32 sequence windows = 1024 transitions per gradient
    step, already >= L's 128 transitions/env-step data throughput.
  - Eval greedy from the start (post eval-epsilon-leak fix, same as O/P).

Option-Critic losses (single backward pass per gradient step):
  Critic: QR-Huber on Q_ω(s,a) with target U = (1−β_ω)·Q_ω(s',a*) + β_ω·V_Ω*(s')
  Actor:  −log π_ω(a|s)·(Q(s,a,ω) − V(s,ω)) − ε_H·H(π_ω)
  Term:   β_ω(s)·(Q_Ω(s,ω) − V_Ω*(s) + ξ)   [encourage termination when suboptimal]

Usage:
    python ablate_q_train.py
"""
...
# One gradient step per this many env steps. A 32-window GRU sequence batch
# costs ~11 ms (probe: 50 calls, 4 torch threads) vs ~1.5 ms/env step, over
# ~1.14M env steps/worker; CI workers hard-cap at 6h. Every-step training
# projects to ~8h at 2x CI slowdown; every-2 to ~4.6h — inside budget, with
# 8x M's gradient-step count (571k vs 71k) and ~4x L's data throughput
# (1024 transitions/gradient step vs L's 128/env step).
TRAIN_EVERY_STEPS = 2
...
BASE_CONFIG = QRConfig(
    ...
    huber_kappa       = 1.0,
    learning_rate     = 1e-4,          # L's value — per-step cadence needs no compensation
    optimizer         = 'adam',
    gamma             = 0.97,
    epsilon_start     = 1.0,
    epsilon_end       = 0.03,
    epsilon_decay     = 0.9988,
    target_update_freq = 15,           # L's value (episodes between hard target copies)
    ...
    oc_entropy_reg     = 0.01,
    oc_term_reg        = 0.01,
    grad_clip_norm     = None,         # L has no clipping; P showed it never engages
)

WEIGHTS_DIR    = Path(__file__).parent / 'weights'
LOG_PATH       = Path('/tmp/train_ablate_q.log')
...
def worker_fn(worker_id: int, result_queue: mp.Queue, max_episodes: int = 0):
    ...
    for ep in range(n_eps):
        ...
        while not done:
            ...
            # Per-step training (the change vs M/O/P): sample sequence windows
            # from previously completed episodes while this one unrolls.
            env_steps += 1
            if env_steps % TRAIN_EVERY_STEPS == 0 and seq_buf.can_sample(config.batch_size):
                if agent.train_batch_options(seq_buf) is not None:
                    n_grad_steps += 1
...
def main():
    ...
    tee('═' * 78)
    tee('ABLATION Q: option-critic + GRU, per-step cadence — v2 server, 24-dim state, 3600 eps')
    tee('═' * 78)
    ...
    tee(f'Change vs M: per-step cadence (1 grad step / {TRAIN_EVERY_STEPS} env steps), '
        f'lr={c.learning_rate}, target_update={c.target_update_freq}, '
        f'grad_clip={c.grad_clip_norm} — architecture untouched')
    tee('─' * 78)
    ...
    summary = {
        'ablation':              'Q_per_step_option_critic',
        'winner_worker':         wid,
