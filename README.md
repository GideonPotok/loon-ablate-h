...
| Q | Tests P's cadence hypothesis directly: M's exact architecture + L's per-step training cadence (every 2 env steps ⇒ 8× M's gradient count), lr 1e-4, target-update 15, no clipping | **25.0%** on a clean 10-seed re-eval — statistically indistinguishable from M's 24.7%, despite 8× the gradient steps. Refutes the cadence-starvation hypothesis: option-critic's shortfall vs. L (46.9%) is not fixed by more gradient steps alone |
...
- **Q's own CI summary (`dqn_ablate_q_summary.json`) reports `best_score: 37.9%`** — a single checkpoint's in-training eval (3 seeds/preset, `EVAL_RUNS=3`). The **25.0%** cited above is a clean 10-seed-per-preset re-eval of that same checkpoint (`probe_realism_transfer.py --ablation q --weights weights/dqn_ablate_q.pt`, legacy-flags mode), matching the methodology used for L/M's numbers. Use 25.0% for cross-ablation comparison; 37.9% overstates Q by picking up favorable low-seed-count variance (per-preset stdev on the 10-seed eval is ±11–12 pp).
...
### Ablation Q (option-critic, per-step cadence — see caveat, 25.0% clean re-eval)

| tropical | strong-shear | calm |
|----------|-------------|------|
| ![Q tropical](replay_ablate_q_tropical.gif) | ![Q strong-shear](replay_ablate_q_strong_shear.gif) | ![Q calm](replay_ablate_q_calm.gif) |
