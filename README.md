...
| Ablation | Arm | Result (10-worker CI matrix, 3600 eps/worker) |
|---|---|---|
| R | Floor: N's recipe, no time features (state 20-dim) — no phase information at all | Best **64.2%** (tropical 77.6 / strong-shear 52.0 / calm 100), worker mean 44.9% ± 8.8 pp, all 10 workers ≥ 36.5%. The realism env is learnable without the oracle — far above the ~7% random floor and the probe's 16% transfer bound |
| S | Estimation: `use_estimated_phase_features` — online demodulation of GPS-drift wind residuals → 4 real-life-computable phase features (state 24-dim) | Best 57.3%, worker mean **50.5% ± 5.7 pp** — +5.6 pp mean over R (~1.7σ, suggestive not conclusive) and visibly more consistent, but a lower observed ceiling than R's best |
| T | Memory: GRU-64 (`use_recurrent=True`, no options) with R2D2-style sequence replay (burn-in 16 + train 16), Q's per-step cadence | Best 41.4%, worker mean **30.4% ± 6.6 pp** — significantly *below* the feedforward floor (−14.5 pp vs R, ~4σ); calm preset collapses (best 45.2% vs R's 100%) |

**Readout vs the pre-registered decision rules:** the hypothesis was S ≫ R (engineered phase estimation recovers what the oracle clock provided), with T telling us whether memory could learn the same inference for free. Instead: **S ≈ R (modest +)** — explicit estimation buys a small mean/consistency gain, not an oracle-sized one, so phase information is not the load-bearing ingredient once the env stops rewarding clock-reading. And **T < R** — the recurrent arm does not merely fail to learn phase inference, it underperforms the feedforward baseline outright (T's *best* worker, 41.4%, sits below R's worker *mean*), extending the M/O/P/Q pattern: recurrent/option machinery has never paid for itself on this problem. Caveat on T: the 2h curriculum tier's episodes (24 nav steps) are shorter than the 32-step sequence window and contribute no gradient steps — inherited from Q's machinery; 100 of 3600 episodes.

Raw artifacts: `weights/dqn_ablate_{r,s,t}.pt` + `_summary.json` (untracked, per the J–N convention). S and T trained in a separate org for Actions-concurrency parallelism ([Minevra-co/loon-ablate-s](https://github.com/Minevra-co/loon-ablate-s), [Minevra-co/loon-ablate-t](https://github.com/Minevra-co/loon-ablate-t)); code PRs in this repo: #3 (R), #4 (S), #6 (T).
