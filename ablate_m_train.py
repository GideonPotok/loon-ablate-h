"""
Ablation M — Option-Critic with GRU (v2 server, 3600 episodes):

Question this answers:
  "Does temporal abstraction via option-critic (4 options, GRU-64 hidden state)
   give the agent the ability to discover and commit to multi-hour altitude
   strategies, fixing the IGW-phase-shift escape failure?"

Background:
  The balloon needs to hold an altitude for 1–4h to assess its quality and to
  transit the 7km band (~47 min from bottom to top). A memoryless DQN picks
  actions every 5 min with no notion of commitment. Option-Critic (Bacon et al.
  2017) learns K options, each with its own intra-option policy π_ω and
  termination function β_ω, allowing the agent to discover altitude-commitment
  strategies without hand-coding them.

  The GRU hidden state accumulates evidence about IGW phase across the episode,
  serving as a compressed sufficient statistic for the POMDP → MDP conversion.

Changes vs Ablation L:
  - use_recurrent=True (GRU-64 hidden state)
  - use_options=True  (4 options, option-critic losses)
  - Sequence replay: EpisodeSequenceBuffer stores full episodes, samples L=32
    windows for burn-in (16) + training (16) pattern.
  - Training: per-episode rather than per-step (K = max(1, T // seq_train)
    gradient steps per episode, where T is episode length in steps).
  - Everything else identical to ablation L: Fourier time features (24-dim
    state), τ=500 km exponential shaping, recovery spawn, 3600 eps/worker.
...
LOG_PATH       = Path('/tmp/train_ablate_m.log')
...
def main():
    ...
    tee('═' * 78)
    tee('ABLATION M: option-critic + GRU — v2 server, 24-dim state, 3600 eps')
    tee('═' * 78)
    ...
    tee(f'Change vs L: GRU-{c.gru_hidden} + {c.n_options} options (option-critic)')
    tee('─' * 78)
