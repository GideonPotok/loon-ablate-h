"""
Ablation P — Option-Critic with GRU, gradient-clipped only (v2, 3600 eps):

Question this answers:
  "Ablation O bundled three changes on top of M (learning_rate 3e-4->1e-4,
   batch_size 32->64, oc_term_reg 0.01->0.05, plus new grad_clip_norm=5.0)
   and the result was *worse* than M, not better — every worker showed a
   flat, noisy score with no real learning curve, unlike M where ~3/10
   workers clearly converged. A local gradient-norm probe (948 steps,
   median 0.74, max 1.26, 0% > 5.0) showed clipping never engages at
   this loss scale, ruling it out as O's cause. The likely real culprit:
   M/O's per-episode training cadence does ~36x fewer gradient steps than
   L's per-step cadence for the same data (4 updates per 6h episode vs
   ~144 for L) — M's learning_rate=3e-4 ('slightly higher — GRU needs
   faster convergence', per M's own comment) was almost certainly a
   correct compensation for that starvation, not a mistake to revert.

  So: is gradient clipping alone — isolated from the LR/batch_size/
  term_reg changes that turned out to be confounds — enough to reduce
  M's seed-to-seed variance (stdev 9.9pp, 7/10 workers never breaking
  30%) without also tanking its mean performance the way O's bundle did?"

Changes vs Ablation M (single variable):
  - grad_clip_norm: None -> 5.0  (the only change)
  - Eval is greedy from the start (M's branch predated the eval ε-greedy
    leak fix; P is built after that fix, so this was never contaminated
    — unlike M's own training-time eval, which was epsilon-contaminated)
  - learning_rate, batch_size, oc_term_reg, target_update_freq, n_options,
    gru_hidden, curriculum, and all env flags are identical to M.
...
BASE_CONFIG = QRConfig(
    ...
    huber_kappa       = 1.0,
    learning_rate     = 3e-4,          # slightly higher — GRU needs faster convergence
    optimizer         = 'adam',
    gamma             = 0.97,
    epsilon_start     = 1.0,
    epsilon_end       = 0.03,
    ...
    oc_entropy_reg     = 0.01,
    oc_term_reg        = 0.01,
    grad_clip_norm     = 5.0,          # new — the only change vs M
)

WEIGHTS_DIR    = Path(__file__).parent / 'weights'
LOG_PATH       = Path('/tmp/train_ablate_p.log')
WEIGHTS_PREFIX = 'dqn_ablate_p'
...
def main():
    ...
    tee('═' * 78)
    tee('ABLATION P: option-critic + GRU, gradient-clipped only — v2 server, 24-dim state, 3600 eps')
    tee('═' * 78)
    ...
    tee(f'Change vs M: grad_clip_norm={c.grad_clip_norm} (only change; lr/batch/term_reg all match M)')
    tee('─' * 78)
    ...
    summary = {
        'ablation':              'P_clipped_option_critic',
        'winner_worker':         wid,
